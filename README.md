...
* <b>逐张图片/视频位置覆盖（v1.2.1）</b>：移除全局 positionX/Y，每张图片/视频独立设置偏移，数据保存在 `imageOverrides` 中
* <b>分辨率检测禁用无效轴</b>：根据图片/视频实际分辨率与视口宽高比，自动禁用无法生效的 X 或 Y 滑块（`background-size: cover` 下仅一个轴有效）
* <b>视频推荐栏扁平化</b>：取消折叠节点，推荐链接直接可见
* 解决了一些已知的问题
...
感谢 `思源爱好者折腾群` 中各类大佬耐心解答我关于此插件开发中遇到的各种问题，以及DeepSeek V4 Pro在代码理解和重构上的强而有力的帮助。
